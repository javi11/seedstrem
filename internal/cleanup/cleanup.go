...
func New(st *store.Store, dc deluge.Client, svc *torrents.Service, sessions *playsession.Sessions, settings func() Settings, logger *slog.Logger, interval time.Duration) *Cleanup {
	if logger == nil {
		logger = slog.Default()
	}
	if interval <= 0 {
